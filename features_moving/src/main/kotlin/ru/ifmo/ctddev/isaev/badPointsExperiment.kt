...
fun main(args: Array<String>) {
   /* var lRaw = SpacePoint(longArrayOf(2048, 545), 2048) //old experiment, success
    ...
    var lRaw = SpacePoint(longArrayOf(64, 45), 64) //new experiment, still no success
    var rRaw = SpacePoint(longArrayOf(256, 181), 256)
    do {
        println("Left: $lRaw")
        println("Right: $rRaw")
        val mRaw = lRaw.inBetween(rRaw)
        val left = getPointOnUnitSphere(getExactAngle(lRaw))
        val langles = getExactAngle(lRaw)
        println("Left angles: ${Arrays.toString(langles)}")
        val rangles = getExactAngle(rRaw)
        println("Riht angles: ${Arrays.toString(rangles)}")
        val right = getPointOnUnitSphere(rangles)
        val middle = getPointOnUnitSphere(getExactAngle(mRaw))
